...
int   counter[10];
char *Part[6] = {(char *)"pi\0", (char *)"p\0", (char *)"d\0", (char *)"t\0", (char *)"He3\0", (char *)"He4\0"};
...
//int gil_main(char ofilename[], int ArrSize)
{
  ...
  printf("Running the program for positive pions of mean momentum %f MeV/c\n", P_MEAN);
  ...
  delete gROOT->GetListOfCanvases()->FindObject("c4");
  ...
  for(track = 0; track<tracks; track++)
		...
		view = 0;
	{
	  ...
	  Signe=-1; //! for Pi^- we don't know the signe of result tracks
	...
	RENEWFIT0:
	  ...
	  if(track == 0 && renew == 0) 
	    {
	      double p0 = 0.03*MFIELD*rcD[view][track]*sqrt(1.+pow(a[view][track]/rcD[view][track],2))
...
void read_data()
{
  ...
  delete gROOT->GetListOfCanvases()->FindObject("cp");
  ...
  //read_ascii();
  delete gROOT->GetListOfCanvases()->FindObject("cw");
  cw = new TCanvas("cw","br along radius",25,25,1000,800);
  cw->Divide(2,1);

  gStyle->cd();
  cw->SetFillColor(0);

  delete gROOT->GetListOfCanvases()->FindObject("csl");
  csl = new TCanvas("csl","br along radius-background",20,20,1000,800);
  csl->Divide(2,1);
		...
		for(itrack=0; itrack<tracks; itrack++)
		  {
			 ...
			 */
			 ...
			 //TH1F *c = 0;                                                               
			 //c = (TH1F*)gROOT->FindObject("hwidth"); if (c) c->Delete(); c = 0;  
...
TF1 *myfit = new TF1("myfit","[0]*exp(-0.5*pow((x-[4])/[1],2))+[2]+[3]*x+[5]*exp(-0.5*pow((x-[6])/[7],2))", (float) -(iwidth), (float) iwidth);
  ...
  //  Double_t ch = fit->GetChisquare();    // unused
  *sigma = (float) fit->GetParameter(1);
  ...
  //  Double_t B = fit->GetParameter(2);    // unused
  //  Double_t A = fit->GetParameter(3);    // unused
