...
void read_data()
{
  ...
  cp->Update();
  c2->cd();
  TPaveLabel *pbr = new TPaveLabel(0.61,0.01,0.99,0.3,eventname,"br");
  pbr->SetFillColor(42);
  pbr->SetTextSize(0.3);
  pbr->Draw();
  char PStr[60];
  TPaveText *textbr = new TPaveText(0.61, 0.01, 0.99, 0.3 );
			      ...
			      float x1, float y1, float x2, float y2, int ima, int itr, float *sigma, float *shift)
{
  ...
  hwidth->Draw();
  //TF1 *c = 0;                                                               
  //c = (TF1*)gROOT->FindObject("myfit"); if (c) c->Delete(); c = 0;  
...
TF1 *myfit = new TF1("myfit","[0]*exp(-0.5*pow((x-[4])/[1],2))+[2]+[3]*x+[5]*exp(-0.5*pow((x-[6])/[7],2))", (float) -(iwidth), (float) iwidth);
  ...
  //c = (TF1*)gROOT->FindObject("fit"); if (c) c->Delete(); c = 0;  
